        ...
        <div class="mt-4 relative">
            ...
            @auth
                <div class="inline-flex">
                    <button id="favoriteOpenModal" class="text-end float-right absolute top-0 right-0 mt-1">
                        ...
                        <div id="menu" class="w-full h-full bg-gray-900 bg-opacity-80 top-0 fixed sticky-0">
                            <div class="2xl:container  2xl:mx-auto py-48 px-4 md:px-28 flex justify-center items-center">
                                <div class="w-96 md:w-auto dark:bg-gray-800 relative flex flex-col justify-center items-center bg-white py-16 px-4 md:px-24 xl:py-24 xl:px-36">
                                    <button id="closeFavoriteModal" type="button"
                                    ...
                                    <div class="text-center">
                                        <a href="{{ route('movies.show', $movie['id']) }}" target="_blank">
                                            ...
                                            <h3 class="text-lg font-normal text-gray-500 dark:text-gray-400">{{ $movie['title'] }}</h3>
                                        </a>
                                        ...
                                        <div class="flex items-center text-gray-400 text-sm mt-1 mb-2">
                                            ...
                                            <span class="text-sm font-bold text-gray-500 dark:text-gray-400 ml-1">{{ $movie['vote_average'] }}</span>
                                            <span class="text-sm font-bold text-gray-500 dark:text-gray-400 mx-2">|</span>
                                            <span class="text-sm font-bold text-gray-500 dark:text-gray-400">{{ $movie['release_date'] }}</span>
                                            <span class="text-sm font-bold text-gray-500 dark:text-gray-400 mx-2">|</span>
                                            <span class="text-sm font-bold text-gray-500 dark:text-gray-400">{{ $movie['genres'] }}</span>
                                        </div>


                                        @if(count($movie['lists']) > 0)
                                            <div class="inline-flex items-center text-gray-400 text-sm mt-6 my-2">


                                                <form action="/favorites" method="POST">
                                                    @csrf
                                                    <input type="hidden" name="show_category" value="movies">
                                                    <input type="hidden" name="show_id" value="{{ $movie['id'] }}">
                                                    ...
                                                    <input type="hidden" name="show_genre"
                                                           value="{{ $movie['genres'] }}">
                                        ...
                                        @else
                                            <div class="items-center text-gray-400 text-sm mt-12 mb-2">
                                                <p><a class="text-lg font-normal text-gray-500 dark:text-gray-400 hover:text-orange-500 transition ease-in-out duration-200"
                                                      href="{{ route('create-your-list.create') }}">Create your first
                                                        list.</a></p>
                                                <p><small class="text-gray-500 dark:text-gray-400">Create a list so you
                                                        can add show in favorite.</small></p>
                                            </div>
                                        @endif
                                    </div>
                                </div>
