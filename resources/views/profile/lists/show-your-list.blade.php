<x-app-layout>
    <x-slot name="header">
        ...
        <a class="text-sm hover:text-orange-500 transition ease-in-out duration-200"
           href="{{ route('your-lists.index') }}">Back to lists</a>
    </x-slot>
    <div class="py-6">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                                    ...
                                    @if (count($favorites) > 0)
                                        @foreach($favorites as $favorite)
                                            @if ($favorite->user_id === auth()->user()->id && $favorite->list_id === $lists->id)
                                                <div class="mt-2">
                                                    <a href="/{{ $favorite->show_category.'/'.$favorite->show_id }}">
                                                        <img src="{{ $favorite->show_image }}"
                                                    ...
                                                    <div class="mt-2 relative">
                                                        ...
                                                        <div class="flex items-center text-gray-400 text-sm mt-1">
                                                            ...
                                                            <span class="ml-1 text-xs">{{ $favorite->show_rating }}</span>
                                                            <span class="mx-2 text-xs">|</span>
                                                            <span class="text-xs">{{ $favorite->show_date }}</span>
                                                        </div>
